...
import com.example.eala.databinding.FragmentDendometroBinding
...
// pantalla de las mediciones del dendómetro.

class MedicionesDendometroFragment : Fragment() {

    // Creamos una variable para el binding. Por si al entrar en la vista no se crea. Y manejar mejor
    // la memoria.
    private var _binding: FragmentDendometroBinding? = null
    ...
    override fun onCreateView(
        ...
        // Inflamos el lyout
        _binding = FragmentDendometroBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModelMediciones.dataPoints.observe(viewLifecycleOwner) { datos ->
            setDataToDendrometroChart(datos)
        }
    }
    ...
    fun setDataToDendrometroChart(dataPoints: List<DataPoint>) {
        val entries = mutableListOf<Entry>() // Aquí alojamos las mediciones.
        ...
        dataPoints.forEachIndexed { index, dp ->
            ...
            entries.add(Entry(index.toFloat(), dp.variation.toFloat()))
        }

        // Creamos un DataSet para contenerlas. Les damos color verde.
        val dataSet = LineDataSet(entries, "Variación (mm)").apply {
            color = resources.getColor(R.color.verde_fondo, null)
            setDrawCircles(false)
            lineWidth = 2f
        }

        val lineData = LineData(dataSet)
        // Cargamos el dataset a la gráfica.
